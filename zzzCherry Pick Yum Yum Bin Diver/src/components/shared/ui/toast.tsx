...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {children}
      {/* Keep toasts above dialogs/modals/call overlays/backdrops */}
      <div className="fixed top-4 right-4 z-[2147483647] space-y-2 pointer-events-none">
        <AnimatePresence>
          {toasts.map((toast) => (
            <motion.div
              key={toast.id}
              ...
              className={`glass-card border ${getColors(toast.type)} p-4 min-w-[300px] max-w-md pointer-events-auto`}
            >
