...
import { PROFILE_SECTIONS, calculateSectionCompletion, getSectionCompletionColor, type ProfileSection } from '@/lib/profile-sections'
...
export default function CandidateProfilePage() {
  ...
  // AUTO-SAVE STATE - Facebook-style robust saving
  const [autoSaving, setAutoSaving] = useState(false)
  const [lastSaved, setLastSaved] = useState<Date | null>(null)
  const [savingSection, setSavingSection] = useState<ProfileSection | null>(null)
  const [sectionStatus, setSectionStatus] = useState<Record<ProfileSection, 'saved' | 'saving' | 'error' | null>>({
    basic: null,
    location: null,
    work: null,
    social: null,
  ...
  async function fetchProfile() {
    ...
    try {
      ...
      // Build base form data with proper defaults and type conversions
      const baseData = {
  ...
  const handlePhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Don't refresh profile - it resets form fields the user is editing
      // Just update the avatar URL in state
      if (profile) {
  ...
  const handleCoverPhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log('🖼️ Starting cover photo upload...')
      ...
      console.log('✅ Cover photo uploaded to Supabase:', publicUrl)
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Failed to update cover photo:', response.status, errorText)
      ...
      console.log('✅ Cover photo updated successfully:', publicUrl)
      ...
      // Don't refresh profile - it resets form fields the user is editing
      // Just update the cover photo URL in state
      if (profile) {
        setProfile({ ...profile, cover_photo: publicUrl })
      }
  ...
  const parseLocation = async (locationText: string) => {
    ...
    try {
      ...
      console.log('🤖 Parsing location with AI:', locationText)
      ...
      if (result.success && result.data) {
        console.log('✅ AI parsed location:', result.data)
        ...
        // Trigger auto-save for location section
        triggerAutoSave('location')
      }
    } catch (error) {
  ...
  // AUTO-SAVE - Facebook-style debounced saving with retry logic
  const triggerAutoSave = useCallback((section: ProfileSection, dataOverride?: typeof formData) => {
    if (!AUTO_SAVE_ENABLED) {
      return
    }
    if (!user?.id) {
      console.warn('⚠️ Skipping auto-save: no user ID available')
      return
    }
    const dataToSave = dataOverride ?? formDataRef.current
    ...
    autoSaveTimeoutRef.current = setTimeout(() => {
      executeSectionSave(section, dataToSave)
  ...
  const normalizeProfileValue = (key: keyof typeof formData, value: any) => {
    switch (key) {
      ...
      case 'current_mood':
        return value === 'prefer_not_to_say' || value === '' || value === null || value === undefined
          ? null
          : String(value).trim()
      case 'gender_custom':
  ...
  // Execute section-based save with retry logic
  const executeSectionSave = useCallback(async (
    section: ProfileSection,
    dataOverride?: typeof formData,
    options?: { force?: boolean }
  ) => {
    if (!AUTO_SAVE_ENABLED && !options?.force) {
      return
    }
    if (!user?.id) {
      console.warn('⚠️ Skipping auto-save: no user ID available')
    ...
    const dataSnapshot = dataOverride ?? formDataRef.current
    // If already saving, queue this save
    if (isSavingRef.current) {
      console.log(`⏳ Save in progress, queuing ${section} section...`)
      saveQueueRef.current = { section, data: dataSnapshot }
      return
    }
    ...
    try {
      ...
      console.log(`📊 Current form data snapshot:`, {
        section,
        formDataKeys: Object.keys(dataSnapshot),
        sampleValues: {
          phone: dataSnapshot.phone,
          birthday: dataSnapshot.birthday,
          gender: dataSnapshot.gender,
          position: dataSnapshot.position,
          work_status: dataSnapshot.work_status,
        },
      })

      // Prepare section-specific data
      const sectionFields = PROFILE_SECTIONS[section].fields
      const sectionData: any = {}

      // Collect only fields for this section, excluding username (it goes to candidates table)
      // Normalize values to avoid invalid payloads (e.g., empty date, bad numbers)
      // IMPORTANT: Include null values in payload to allow clearing fields in database
      sectionFields.forEach(field => {
        if (field.key !== 'username') {
          let value = dataSnapshot[field.key as keyof typeof formData]
          const normalized = normalizeProfileValue(field.key as keyof typeof formData, value)
          // Always include the field, even if null (to allow clearing database values)
          sectionData[field.key] = normalized
        }
      })

      console.log(`📦 Prepared payload for ${section} section:`, {
        sectionFields: sectionFields.map(f => f.key),
        payloadKeys: Object.keys(sectionData),
        payload: sectionData,
        payloadSize: JSON.stringify(sectionData).length,
      })
      ...
      if (section === 'basic' && dataSnapshot.username) {
        // Basic section: Update username in candidates table first
        const candidateUpdate = {
          username: dataSnapshot.username.toLowerCase().trim(),
          slug: slugify(dataSnapshot.username.toLowerCase().trim()),
        }
        candidateResponse = await fetch(`/api/candidates/${user?.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (!candidateResponse.ok) {
          ...
          throw new Error(errorData.details || errorData.error || 'Failed to update username')
        }
      }

      // Save profile fields (username already filtered out above)
      if (Object.keys(sectionData).length > 0) {
        console.log(`🌐 Sending ${section} section to API...`)
        profileResponse = await fetch(`/api/candidates/${user?.id}/profile`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
        ...
        // If no profile data to save (e.g., only username in basic section), skip profile update
        console.log(`✓ No profile fields to save for ${section} section`)
      }

      // Only check response if we made a profile update
      if (profileResponse && !profileResponse.ok) {
        const errorData = await profileResponse.json().catch(() => ({}))
        console.error(`❌ API error for ${section} section:`, errorData)
        throw new Error(errorData.details || errorData.error || 'Save failed')
      }

      if (profileResponse && profileResponse.ok) {
        const successData = await profileResponse.json().catch(() => ({}))
        console.log(`✅ API success for ${section} section:`, {
          profile_id: successData.profile?.id,
          candidate_id: successData.profile?.candidate_id,
        })
      }
      ...
      if (saveQueueRef.current) {
        ...
        setTimeout(() => executeSectionSave(queued.section, queued.data, options), 100)
      }

    } catch (error) {
      console.error(`❌ Auto-save failed for ${section}:`, error)

      // Retry logic (up to 3 times)
      if (retryCountRef.current < 3) {
        retryCountRef.current++
        console.log(`🔄 Retrying ${section} save (${retryCountRef.current}/3)...`)
        setTimeout(() => executeSectionSave(section, undefined, options), 2000 * retryCountRef.current)
      } else {
        retryCountRef.current = 0
        ...
        toast({
          ...
          description: `Failed to save ${PROFILE_SECTIONS[section].title}. Your data is backed up locally.`,
          variant: 'destructive',
        })
  ...
  const handleSectionSave = useCallback((section: ProfileSection) => {
    executeSectionSave(section, formDataRef.current, { force: true })
  }, [executeSectionSave])
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (Object.keys(candidateUpdate).length > 0) {
        ...
        if (!candidateResponse.ok) {
          const errorData = await candidateResponse.json().catch(() => ({}))
          console.error('❌ Candidate update failed:', errorData)
      ...
      fetchProfile().catch(err => {
        console.error('⚠️ Error reloading profile after save:', err)
      })
    ...
    <TooltipProvider>
      <div className="space-y-6 pb-8">
        ...
        <div className="relative rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.07] group">
          ...
          {/* BASIC INFORMATION SECTION HEADER - with auto-save status */}
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                disabled={shouldDisableFields || sectionStatus.basic === 'saving'}
                variant="outline"
                className="border-white/10 text-gray-300 hover:bg-white/5 hover:border-white/20 h-9 px-4"
              >
                {sectionStatus.basic === 'saving' ? (
                  <>
                    <Loader2 className="w-4 h-4 mr-2 animate-spin" />
              ...
              {(() => {
                const completion = calculateSectionCompletion('basic', formData)
                const colors = getSectionCompletionColor(completion.percentage)
                return (
                  <div className={cn("px-3 py-1.5 rounded-lg border flex items-center gap-2", colors.bg, colors.border)}>
                    {completion.percentage === 100 ? (
                      <>
                        <CheckCircle className={cn("w-4 h-4", colors.icon)} />
                        <span className={cn("text-sm font-semibold", colors.text)}>Complete</span>
                      </>
                    ) : (
                      <>
                        <Target className={cn("w-4 h-4", colors.icon)} />
                        <span className={cn("text-sm font-semibold", colors.text)}>{completion.percentage}%</span>
                      </>
                    )}
                  </div>
                )
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    handleFieldChange('username', e.target.value, 'basic')
                  }}
                  placeholder="johndoe"
                  className={cn(
                    inputClass,
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleFieldChange('headline', e.target.value, 'basic')}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) => handleFieldChange('phone', e.target.value, 'basic')}
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <Select
                  ...
                  onValueChange={(month) => {
                    ...
                    handleFieldChange('birthday', newBirthday, 'basic')
                  }}
                  disabled={shouldDisableFields}
                >
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  handleFieldChange('gender', value, 'basic')
                }}
                disabled={shouldDisableFields}
              >
                <SelectTrigger className={inputClass}>
              ...
              {formData.gender === 'other' && (
                <Input
                  ...
                  onChange={(e) => {
                    handleInputChange('gender_custom', e.target.value)
                    handleFieldChange('gender_custom', e.target.value, 'basic')
                  }}
                  disabled={shouldDisableFields}
                />
              )}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) => handleFieldChange('position', e.target.value, 'basic')}
                  placeholder="e.g., Customer Service Representative"
                  className={cn(inputClass, "pl-10")}
                  disabled={shouldDisableFields}
                />
            ...
            <div className="md:col-span-2">
              ...
              <Textarea
                id="bio"
                value={formData.bio}
                onChange={(e) => handleFieldChange('bio', e.target.value, 'basic')}
                rows={4}
        ...
        <div className="relative rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.07] group">
          ...
          {/* LOCATION SECTION HEADER - with auto-save status */}
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-3">
                  ...
                  {/* AI Powered Badge */}
                  <div className="hidden md:flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border border-cyan-500/30">
            ...
            <div className="flex items-center gap-3">
              <Button
                onClick={() => handleSectionSave('location')}
                disabled={shouldDisableFields || sectionStatus.location === 'saving'}
                variant="outline"
                className="border-white/10 text-gray-300 hover:bg-white/5 hover:border-white/20 h-9 px-4"
              >
                {sectionStatus.location === 'saving' ? (
              ...
              {(() => {
                const completion = calculateSectionCompletion('location', formData)
                const colors = getSectionCompletionColor(completion.percentage)
                return (
                  <div className={cn("px-3 py-1.5 rounded-lg border flex items-center gap-2", colors.bg, colors.border)}>
                    {completion.percentage === 100 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) => handleFieldChange('location', e.target.value, 'location')}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleFieldChange('location_barangay', e.target.value, 'location')}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleFieldChange('location_region', e.target.value, 'location')}
        ...
        <div className="relative rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.07] group">
          ...
          {/* WORK PREFERENCES SECTION HEADER - with auto-save status */}
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <Button
                onClick={() => handleSectionSave('work')}
                disabled={shouldDisableFields || sectionStatus.work === 'saving'}
                variant="outline"
                className="border-white/10 text-gray-300 hover:bg-white/5 hover:border-white/20 h-9 px-4"
              >
                {sectionStatus.work === 'saving' ? (
              ...
              {(() => {
                const completion = calculateSectionCompletion('work', formData)
                const colors = getSectionCompletionColor(completion.percentage)
                return (
                  <div className={cn("px-3 py-1.5 rounded-lg border flex items-center gap-2", colors.bg, colors.border)}>
                    {completion.percentage === 100 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => handleFieldChange('work_status', value, 'work')}
                disabled={shouldDisableFields}
              >
                <SelectTrigger className={inputClass}>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => handleFieldChange('preferred_shift', value, 'work')}
                disabled={shouldDisableFields}
              >
                <SelectTrigger className={inputClass}>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => handleFieldChange('preferred_work_setup', value, 'work')}
                disabled={shouldDisableFields}
              >
                <SelectTrigger className={inputClass}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Input
                    ...
                    onChange={(e) => handleFieldChange('expected_salary_min', e.target.value, 'work')}
                ...
                <div>
                  <Input
                    ...
                    onChange={(e) => handleFieldChange('expected_salary_max', e.target.value, 'work')}
        ...
        <div className="relative rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.07] group">
          ...
          {/* SOCIAL LINKS SECTION HEADER - with auto-save status */}
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-purple-500/30">
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                disabled={shouldDisableFields || sectionStatus.social === 'saving'}
                variant="outline"
                className="border-white/10 text-gray-300 hover:bg-white/5 hover:border-white/20 h-9 px-4"
              >
                {sectionStatus.social === 'saving' ? (
              ...
              {(() => {
                const completion = calculateSectionCompletion('social', formData)
                const colors = getSectionCompletionColor(completion.percentage)
                return (
                  <div className={cn("px-3 py-1.5 rounded-lg border flex items-center gap-2", colors.bg, colors.border)}>
                    {completion.percentage === 100 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleFieldChange('github', e.target.value, 'social')}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleFieldChange('portfolio', e.target.value, 'social')}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleFieldChange('twitter', e.target.value, 'social')}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleFieldChange('facebook', e.target.value, 'social')}
