...
export default function InsightArticleClient({ post, relatedPosts, internalLinks = [] }: Props) {
  ...
  const contentWithLinks = sanitizeHeadings(injectLinks(post.content));
  ...
  const MarkdownComponents: any = {
    ...
    h1: ({ node, ...props }: any) => <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold text-white mt-6 md:mt-12 mb-3 md:mb-6 tracking-tight drop-shadow-[0_0_15px_rgba(255,255,255,0.15)]" {...props} />,

    // Enhanced H2 with special handling (Emman's styling)
    h2: ({ node, ...props }: any) => {
      return (
        <div className="mt-8 md:mt-16 mb-4 md:mb-8 group">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white border-b border-white/10 pb-3 md:pb-4 flex items-center gap-2 md:gap-3 relative pl-3 md:pl-4 lg:pl-0" {...props}>
            <span className="absolute left-0 lg:-left-6 top-1 w-1 h-6 md:h-8 bg-gradient-to-b from-cyan-500 to-purple-500 rounded-full opacity-100 shadow-[0_0_10px_rgba(6,182,212,0.5)]" />
    ...
    h3: ({ node, ...props }: any) => {
      ...
      if (isSpecialHeading) {
        return (
          <div className="mt-10 md:mt-16 mb-6 md:mb-8 flex items-start sm:items-center gap-3 sm:gap-5 group relative">
            <div className="relative flex-shrink-0">
              ...
              <div className="relative p-2 sm:p-3 rounded-xl sm:rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm shadow-xl shadow-cyan-500/10 group-hover:border-cyan-500/30 transition-all duration-300">
                <SpecificIcon className="w-6 h-6 sm:w-8 sm:h-8 text-cyan-400 drop-shadow-[0_0_8px_rgba(6,182,212,0.4)]" />
              </div>
            </div>
            <div className="flex flex-col min-w-0">
              <div className="h-px w-8 sm:w-12 bg-gradient-to-r from-cyan-500 to-transparent mb-2" />
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-white tracking-tight leading-tight" {...props}>
                {props.children}
              </h3>
            </div>
          </div>
      ...
      // Default H3 styling
      return <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-cyan-400 mt-6 md:mt-10 mb-3 md:mb-4 tracking-wide flex items-center gap-2 before:content-['#'] before:text-cyan-500/30 before:mr-2" {...props} />;
    },

    h4: ({ node, ...props }: any) => <h4 className="text-xl font-semibold text-white mt-8 mb-3 border-l-2 border-purple-500 pl-4" {...props} />,
    p: ({ node, ...props }: any) => <p className="text-gray-300 leading-relaxed mb-4 md:mb-6 text-[15px] md:text-lg font-light tracking-wide" {...props} />,
    ...
    ul: ({ node, ...props }: any) => {
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mt-4">
            {props.children.map((child: any, index: number) => (
              <div key={index} className="p-3 md:p-4 border border-white/10 rounded-lg shadow-inner shadow-purple-500/5 backdrop-blur-sm">
                {child}
              </div>
            ))}
          </div>
      ...
      // Default card-style UL
      return <ul className="space-y-3 md:space-y-4 my-4 md:my-6 p-4 md:p-6 rounded-xl border border-white/10 bg-white/5 shadow-inner shadow-purple-500/5 backdrop-blur-sm" {...props} />;
    },
    ...
    blockquote: ({ node, ...props }: any) => {
      ...
      return (
        <div className={`flex items-start gap-3 md:gap-4 border-l-4 ${borderColor} bg-gradient-to-r ${bgColor} to-transparent p-3 md:p-5 rounded-r-xl my-4 md:my-6`}>
          <div className={`flex-shrink-0 p-2 md:p-2.5 rounded-lg md:rounded-xl bg-white/5 ${iconColor}`}>
            <CalloutIcon className="w-5 h-5 md:w-6 md:h-6" />
          </div>
          <div className="flex-1 min-w-0">
            {label && <span className={`text-xs font-bold uppercase tracking-wider ${iconColor} block mb-1`}>{label}</span>}
            <p className="text-gray-200 text-sm md:text-base leading-relaxed m-0">{displayContent || props.children}</p>
          </div>
        </div>
      );
    },
    ...
    th: ({ node, ...props }: any) => <th className="p-3 md:p-4 text-cyan-400 text-sm md:text-base" {...props} />,
    td: ({ node, ...props }: any) => <td className="p-3 md:p-4 border-b border-white/5 text-gray-300 text-sm md:text-base group-hover:bg-white/5 transition-colors" {...props} />,
      ...
      <section className="relative pt-20">
        ...
        <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl pt-8 pb-16">
          ...
          >
            ...
            {/* Title */}
            <h1 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-extrabold text-white leading-[1.1] tracking-tight drop-shadow-2xl">
              {post.title}
            </h1>

            {/* Description */}
            <p className="text-base sm:text-lg md:text-2xl text-gray-200 leading-relaxed font-light max-w-3xl drop-shadow-lg">
              {post.description}
            </p>
      ...
      <article className="pb-20 relative z-10">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          <div className="grid lg:grid-cols-12 gap-12 lg:items-start">
            ...
            <div className="lg:col-span-8" ref={contentRef}>
                ...
                {(post.content_part1 || post.content_part2 || post.content_part3) ? (
                  <>
                    ...
                    {post.content_part1 && (
                      ...
                      >
                        {sanitizeHeadings(injectLinks(post.content_part1))}
                      </ReactMarkdown>
                    )}
                    ...
                    {post.content_part2 && (
                      <ReactMarkdown
                        ...
                        components={MarkdownComponents}
                      >
                        {sanitizeHeadings(injectLinks(post.content_part2))}
                      </ReactMarkdown>
                    )}
                    ...
                    {post.content_part3 && (
                      <ReactMarkdown
                        ...
                        components={MarkdownComponents}
                      >
                        {sanitizeHeadings(injectLinks(post.content_part3))}
                      </ReactMarkdown>
                    )}
                  </>
                ) : (
              ...
              <div className="mt-16 pt-8 border-t border-white/10">
                ...
                <div className="flex flex-wrap gap-3">
                  <Button variant="outline" size="sm" className="border-white/10 hover:bg-[#1DA1F2]/10 hover:text-[#1DA1F2] hover:border-[#1DA1F2]/50 transition-all min-h-[44px]" onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(shareUrl)}`, '_blank')}>
                    <Twitter className="w-4 h-4 mr-2" /> Twitter
                  </Button>
                  <Button variant="outline" size="sm" className="border-white/10 hover:bg-[#0A66C2]/10 hover:text-[#0A66C2] hover:border-[#0A66C2]/50 transition-all min-h-[44px]" onClick={() => window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(shareUrl)}`, '_blank')}>
                    <Linkedin className="w-4 h-4 mr-2" /> LinkedIn
                  </Button>
                  <Button variant="outline" size="sm" className="border-white/10 hover:bg-[#1877F2]/10 hover:text-[#1877F2] hover:border-[#1877F2]/50 transition-all min-h-[44px]" onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`, '_blank')}>
            ...
            {/* Right Column: Sidebar — stacks below on mobile, sticky on desktop */}
            <div className="lg:col-span-4">
              <div className="lg:sticky lg:top-20 space-y-6 z-30 self-start">
                {/* CTA Cards */}
                <StickySidebarCTA />
