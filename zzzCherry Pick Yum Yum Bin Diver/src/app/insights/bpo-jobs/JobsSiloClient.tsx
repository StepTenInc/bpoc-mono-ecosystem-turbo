...
import {
  ...
  Zap, Target, Sparkles, Video, Briefcase, MapPin,
  Search, Globe, Building, DollarSign, Shield, Heart
} from 'lucide-react';
...
// Amber color for jobs silo
const siloColor = '#F59E0B';
...
const MarkdownComponents: any = {
  ...
  h2: ({ node, ...props }: any) => (
    <div className="mt-8 md:mt-16 mb-4 md:mb-8 group">
      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white border-b border-white/10 pb-3 md:pb-4 flex items-center gap-2 md:gap-3 relative pl-3 md:pl-0" {...props}>
        <span className="absolute left-0 md:-left-6 top-1 w-1 h-6 md:h-8 rounded-full opacity-100 shadow-lg bg-amber-400" />
  ...
  h3: ({ node, ...props }: any) => (
    <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-amber-400 mt-6 md:mt-10 mb-3 md:mb-4 tracking-wide flex items-center gap-2" {...props} />
  ),
  p: ({ node, ...props }: any) => (
    <p className="text-gray-300 leading-relaxed mb-4 md:mb-6 text-[15px] md:text-lg font-light tracking-wide" {...props} />
  ),
  ul: ({ node, ...props }: any) => (
    <ul className="space-y-3 md:space-y-4 my-4 md:my-6 p-4 md:p-6 rounded-xl border border-amber-500/30 bg-white/5 shadow-inner backdrop-blur-sm" {...props} />
  ),
  li: ({ node, ...props }: any) => (
    <li className="flex items-start gap-3 text-gray-300 group transition-colors duration-200 hover:text-white">
      <span className="mt-2 w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0 shadow-lg" />
      <span className="flex-1 leading-relaxed">{props.children}</span>
    </li>
  ),
  blockquote: ({ node, ...props }: any) => (
    <div className="flex items-start gap-4 border-l-4 border-amber-400 p-5 rounded-r-xl my-6 bg-amber-500/10">
      <p className="text-gray-200 text-base leading-relaxed m-0">{props.children}</p>
    </div>
  ),
  a: ({ node, ...props }: any) => (
    <a className="font-medium transition-colors text-amber-400 underline underline-offset-4 decoration-2 decoration-amber-500/50 hover:decoration-amber-400" {...props} />
  ),
...
function ArticleCard({ article, index }: { article: any; index: number }) {
  ...
  return (
    ...
    >
      <Link href={`/insights/bpo-jobs/${article.slug}`} className="h-full block group">
        <motion.div
          whileHover={{ y: -8, scale: 1.02 }}
          transition={{ duration: 0.4, ease: "easeOut" }}
          className="h-full flex flex-col relative overflow-hidden rounded-3xl border border-amber-500/20 bg-gradient-to-br from-white/10 via-white/5 to-transparent backdrop-blur-xl shadow-2xl hover:border-amber-400/50 hover:shadow-[0_20px_70px_-10px_rgba(245,158,11,0.3)]"
        >
          <div className="h-48 w-full relative overflow-hidden">
            {hasVideo ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-amber-900/40 to-orange-900/20">
                <Briefcase className="w-20 h-20 text-amber-500/40" />
              </div>
            )}
            ...
            {hasVideo && (
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: index * 0.1 + 0.5, type: "spring" }} className="absolute top-4 right-4">
                <Badge className="bg-amber-500/30 backdrop-blur-md border-amber-400/50 text-amber-200 shadow-lg">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} whileHover={{ opacity: 1, y: 0 }} className="absolute bottom-4 left-4 right-4">
              <div className="bg-black/60 backdrop-blur-md rounded-xl p-3 border border-white/10">
                <div className="flex items-center gap-2 text-xs text-amber-300">
                  ...
                  <span>Jobs Guide • {readingTime}</span>
                </div>
              </div>
            </motion.div>
          </div>
          <div className="p-6 flex flex-col flex-grow">
            <motion.h3 className="text-xl font-bold text-white mb-3 group-hover:text-amber-300 transition-colors line-clamp-2" whileHover={{ x: 4 }}>
              {article.title}
            </motion.h3>
            <p className="text-gray-400 text-sm mb-4 line-clamp-3 flex-grow leading-relaxed">
              {article.description?.substring(0, 120) || ''}...
            </p>
            <div className="flex items-center justify-between mt-auto pt-4 border-t border-amber-500/20">
              <motion.div className="flex items-center gap-2" whileHover={{ x: 4 }}>
                <Image src="/Chat Agent/Ate Yna.png" alt="Ate Yna" width={24} height={24} className="rounded-full ring-2 ring-amber-500/30" />
    ...
    <motion.div ref={carouselRef} initial={{ opacity: 0, y: 60 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="relative" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
      <motion.div className="flex items-center justify-between mb-8" initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.2, duration: 0.6 }}>
        <div className="flex items-center gap-4">
          <motion.div whileHover={{ rotate: 5, scale: 1.1 }} className="w-12 h-12 bg-gradient-to-br from-amber-500/30 to-orange-500/30 rounded-2xl flex items-center justify-center border border-amber-400/30">
            <Briefcase className="w-6 h-6 text-amber-400" />
          </motion.div>
          <div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-white">BPO Job Opportunities</h2>
            <p className="text-gray-500 mt-1 text-sm md:text-base">Find your next role in the BPO industry</p>
          </div>
        </div>
        <div className="hidden md:flex items-center gap-3">
          ...
          <motion.div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 border border-white/10">
            <motion.div animate={{ rotate: isAutoPlaying && !isPaused ? 360 : 0 }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }} className="w-2 h-2 rounded-full bg-amber-400" />
      ...
      <motion.div className="flex justify-center gap-2 mt-8" initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.6 }}>
        {Array.from({ length: maxIndex + 1 }).map((_, index) => (
          <motion.button key={index} onClick={() => goToSlide(index)} className={`relative overflow-hidden rounded-full transition-all duration-300 ${index === currentIndex ? 'w-8 h-2 bg-amber-400 shadow-[0_0_20px_-5px_rgba(245,158,11,0.8)]' : 'w-2 h-2 bg-gray-600 hover:bg-gray-500'}`} whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.9 }}>
            {index === currentIndex && <motion.div className="absolute inset-0 bg-gradient-to-r from-amber-300 to-orange-400" layoutId="activeIndicatorJB" transition={{ type: "spring", stiffness: 500, damping: 50 }} />}
          </motion.button>
        ))}
      </motion.div>
      ...
      {isAutoPlaying && !isPaused && (
        <motion.div className="absolute bottom-0 left-0 right-0 h-0.5 bg-white/10 overflow-hidden rounded-full">
          <motion.div className="h-full bg-gradient-to-r from-amber-400 to-orange-400" initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 4, ease: "linear", repeat: Infinity }} />
        </motion.div>
      )}
    </motion.div>
  );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 40, scale: 0.9 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: index * 0.1, duration: 0.8, type: "spring", stiffness: 100 }} viewport={{ once: true }} whileHover={{ y: -8 }} className="h-full">
      <Link href={`/insights/${silo.slug}`} className="block h-full group">
        <motion.div className="relative p-8 rounded-3xl h-full overflow-hidden border border-white/10 bg-gradient-to-br from-amber-500/10 via-white/5 to-transparent backdrop-blur-xl shadow-xl group-hover:border-amber-400/50 group-hover:shadow-[0_20px_60px_-10px_rgba(245,158,11,0.4)]" whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }}>
          <div className="relative z-10">
            <motion.div className="mb-6" whileHover={{ rotate: 5, scale: 1.1 }}>
              <div className="w-16 h-16 bg-gradient-to-br from-amber-500/30 to-orange-500/20 rounded-2xl flex items-center justify-center border border-amber-400/30">
                <Icon className="w-8 h-8 text-amber-400" />
              </div>
            </motion.div>
            <h3 className="text-xl font-bold text-white group-hover:text-amber-300 transition-colors mb-3">{silo.name}</h3>
            <p className="text-gray-400 text-sm line-clamp-4 mb-6 leading-relaxed">{silo.description}</p>
            <div className="flex items-center text-amber-400 text-sm font-medium group-hover:text-amber-300">
...
export default function JobsSiloClient({ silo, pillarPost, articles, pagination, relatedSilos }: JobsSiloClientProps) {
  ...
  return (
    <div className="overflow-x-hidden selection:bg-amber-500/20 selection:text-amber-200 font-sans min-h-screen bg-[#0B0B0D]">
      <Header />

      {/* Background effects — jobs theme with amber/orange */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <motion.div className="absolute top-[-20%] left-1/3 w-[800px] h-[800px] bg-amber-500/8 rounded-full blur-[150px]" animate={{ scale: [1, 1.1, 1], opacity: [0.8, 1, 0.8] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} />
        <motion.div className="absolute top-[40%] right-[-5%] w-[600px] h-[600px] bg-orange-500/6 rounded-full blur-[120px]" animate={{ scale: [1, 1.2, 1], opacity: [0.6, 1, 0.6] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 2 }} />
        <motion.div className="absolute bottom-[-10%] left-[-10%] w-[700px] h-[700px] bg-yellow-500/5 rounded-full blur-[140px]" animate={{ scale: [1, 1.15, 1], opacity: [0.5, 0.8, 0.5] }} transition={{ duration: 12, repeat: Infinity, ease: "easeInOut", delay: 4 }} />
        
        <div className="absolute inset-0">
          {Array.from({ length: 20 }).map((_, i) => (
            <motion.div key={i} className="absolute w-1 h-1 bg-amber-400/20 rounded-full" style={{ left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%` }} animate={{ y: [0, -30, 0], opacity: [0, 1, 0] }} transition={{ duration: 4 + Math.random() * 4, repeat: Infinity, delay: Math.random() * 4, ease: "easeInOut" }} />
          ))}
        </div>

        <motion.div className="absolute top-[15%] left-[15%] text-amber-400/20" animate={{ y: [0, -15, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}>
          <Search size={40} />
        </motion.div>
        <motion.div className="absolute top-[15%] right-[15%] text-orange-400/20" animate={{ y: [0, 15, 0] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}>
          <Globe size={35} />
        </motion.div>
      </div>
        ...
        <div className="pt-24 pb-8">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="flex items-center gap-2 text-sm text-gray-500 mb-8">
              ...
              <span className="text-amber-400 font-semibold">BPO Jobs</span>
            </motion.div>
          </div>
        </div>
            ...
            <div className="max-w-5xl">
              <motion.div initial={{ opacity: 0, y: 60 }} animate={isHeroInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.3, duration: 0.8 }}>
                <motion.div initial={{ scale: 0, rotate: -10 }} animate={isHeroInView ? { scale: 1, rotate: 0 } : {}} transition={{ delay: 0.5, type: "spring", stiffness: 200 }}>
                  <Badge className="mb-6 md:mb-8 bg-amber-500/20 text-amber-300 border-amber-400/40 text-sm md:text-lg px-4 py-2 md:px-8 md:py-3 shadow-[0_0_30px_-5px_rgba(245,158,11,0.6)]">
                    <Briefcase className="w-4 h-4 md:w-5 md:h-5 mr-2 md:mr-3" />
                    BPO Jobs Hub
                ...
                <motion.h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-9xl font-black mb-6 md:mb-8 tracking-tight" initial={{ opacity: 0, y: 40 }} animate={isHeroInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.6, duration: 0.8 }}>
                  ...
                  <br />
                  <motion.span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-orange-400 to-yellow-500" animate={{ backgroundPosition: ["0% 50%", "100% 50%", "0% 50%"] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} style={{ backgroundSize: "200% 200%" }}>
                    Dream Job.
                  </motion.span>
                </motion.h1>
                ...
                <motion.p className="text-lg md:text-xl lg:text-2xl text-gray-300 mb-6 md:mb-10 max-w-4xl leading-relaxed" initial={{ opacity: 0, y: 30 }} animate={isHeroInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.8, duration: 0.8 }}>
                  Job hunting guides, application tips, and insights into the hottest BPO roles in the Philippines.
                  <span className="text-amber-400 font-semibold"> Your opportunity awaits.</span>
                </motion.p>
                ...
                <motion.div className="flex flex-wrap items-center gap-6 text-lg" initial={{ opacity: 0, y: 30 }} animate={isHeroInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 1, duration: 0.8 }}>
                  <motion.div className="flex items-center gap-2 md:gap-3 bg-amber-500/20 px-4 py-2 md:px-8 md:py-4 rounded-full border border-amber-500/30 backdrop-blur-md shadow-[0_0_30px_-5px_rgba(245,158,11,0.6)]" whileHover={{ scale: 1.05, y: -2 }}>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="grid lg:grid-cols-[1fr_350px] gap-12 items-start">
            {pillarPost && (
              <motion.section ref={pillarRef} initial={{ opacity: 0, y: 60 }} animate={isPillarInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="relative">
                <motion.div className="rounded-3xl border border-amber-500/20 bg-gradient-to-br from-amber-900/20 via-white/5 to-transparent backdrop-blur-xl overflow-hidden shadow-2xl" whileHover={{ y: -4 }} transition={{ duration: 0.3 }}>
                  <div className="p-4 md:p-8 border-b border-amber-500/20 bg-gradient-to-r from-amber-500/10 to-orange-500/10">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <motion.div className="flex items-center gap-4 mb-6" whileHover={{ x: 4 }}>
                          <div className="w-12 h-12 bg-gradient-to-br from-amber-500/30 to-orange-500/20 rounded-2xl flex items-center justify-center border border-amber-400/30 shadow-[0_0_20px_-5px_rgba(245,158,11,0.5)]">
                            <BookOpen className="w-6 h-6 text-amber-400" />
                          </div>
                          <Badge className="bg-amber-500/30 text-amber-200 border-amber-400/40 px-4 py-2">
                            <Sparkles className="w-4 h-4 mr-2" />
                            The Complete Jobs Guide
                          </Badge>
                        </motion.div>
                        <motion.h2 className="text-3xl md:text-4xl font-black text-white mb-3" whileHover={{ x: 4 }}>{pillarPost.title}</motion.h2>
                        <p className="text-gray-400">Everything you need to land your ideal BPO job</p>
                      </div>
                      <motion.button onClick={() => setShowFullPillar(!showFullPillar)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="p-4 rounded-2xl bg-white/5 border border-white/10 text-amber-400 hover:text-white transition-all backdrop-blur-sm">
                    ...
                    <motion.div animate={{ height: showFullPillar ? "auto" : "400px" }} transition={{ duration: 0.5, ease: "easeInOut" }} className="relative overflow-hidden">
                      <div className="prose prose-invert prose-lg max-w-none">
                        {showFullPillar && (pillarPost.content_part1 || pillarPost.content_part2 || pillarPost.content_part3) ? (
                          <>
                            {pillarPost.content_image0 && <figure className="my-8 rounded-2xl overflow-hidden border border-amber-500/20 shadow-2xl"><img src={pillarPost.content_image0} alt={`${pillarPost.title} - Introduction`} className="w-full h-auto object-cover" loading="lazy" /></figure>}
                            ...
                            {pillarPost.content_image1 && <figure className="my-12 rounded-2xl overflow-hidden border border-amber-500/20 shadow-2xl"><img src={pillarPost.content_image1} alt={`${pillarPost.title} - Main content`} className="w-full h-auto object-cover" loading="lazy" /></figure>}
                            ...
                            {pillarPost.content_image2 && <figure className="my-12 rounded-2xl overflow-hidden border border-amber-500/20 shadow-2xl"><img src={pillarPost.content_image2} alt={`${pillarPost.title} - Conclusion`} className="w-full h-auto object-cover" loading="lazy" /></figure>}
              ...
              <div className="space-y-6 max-h-[calc(100vh-120px)] overflow-y-auto">
                ...
                <motion.div className="p-6 rounded-2xl bg-gradient-to-br from-white/5 to-white/[0.02] border border-white/10 backdrop-blur-xl" whileHover={{ y: -4 }}>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-gray-400 text-sm">Job Guides</span>
                      <span className="text-amber-400 font-bold">{pagination.total}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-gray-400 text-sm">Focus Area</span>
                      <span className="text-white font-bold text-sm">PH BPO Market</span>
                    </div>
                    <div className="flex items-center justify-between">
            ...
            <motion.section ref={relatedRef} initial={{ opacity: 0, y: 80 }} animate={isRelatedInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mt-24 mb-16">
              <div className="mb-12 text-center">
                <h2 className="text-4xl md:text-5xl font-black text-white flex items-center justify-center gap-4 mb-4">
                  <motion.div whileHover={{ rotate: 15 }} className="w-12 h-12 bg-gradient-to-br from-amber-500/30 to-orange-500/20 rounded-2xl flex items-center justify-center border border-amber-400/30">
          ...
          <motion.section ref={ctaRef} initial={{ opacity: 0, y: 80 }} animate={isCtaInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-16">
            <motion.div className="max-w-4xl mx-auto text-center relative" whileHover={{ y: -8 }} transition={{ duration: 0.4 }}>
              ...
              <div className="relative p-6 md:p-12 rounded-2xl md:rounded-3xl border border-amber-500/20 bg-gradient-to-br from-amber-900/20 via-white/5 to-transparent backdrop-blur-xl">
                ...
                <motion.p className="text-gray-400 text-base md:text-xl mb-6 md:mb-10 max-w-2xl mx-auto" initial={{ opacity: 0, y: 20 }} animate={isCtaInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.4 }}>
                  Join thousands of professionals who've landed their ideal BPO positions
                </motion.p>
