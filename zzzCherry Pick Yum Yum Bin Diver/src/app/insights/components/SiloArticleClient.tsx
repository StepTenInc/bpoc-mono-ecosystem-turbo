...
export default function SiloArticleClient({ post, relatedPosts, siloSlug, siloName, siloColor = '#06B6D4' }: SiloArticleClientProps) {
  ...
  // The page title is the only H1; article body should start at H2
  const sanitizeHeadings = (content: string) => {
    return content
      // Convert lines starting with single # (but not ## or more) to ##
      .replace(/^# (?!#)/gm, '## ')
      // Remove any duplicate title line that matches the post title
      .replace(new RegExp(`^## ${post.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*$`, 'gm'), '');
  };
  ...
  const MarkdownComponents: any = {
    // H1 in markdown content gets downgraded to H2 — the page title is the only H1
    h1: ({ node, ...props }: any) => (
      <div className="mt-10 md:mt-16 mb-6 md:mb-8 group">
        <h2 className="text-2xl md:text-3xl font-bold text-white border-b border-white/10 pb-4 flex items-center gap-3 relative pl-4 md:pl-0" {...props}>
          <span
            className="absolute left-0 md:-left-6 top-1 w-1 h-8 rounded-full opacity-100 shadow-lg"
    ...
    p: ({ node, ...props }: any) => (
      <p className="text-gray-300 leading-relaxed mb-5 md:mb-6 text-base md:text-lg font-light tracking-wide" {...props} />
    ),
    ul: ({ node, ...props }: any) => (
      <ul
        className="space-y-3 md:space-y-4 my-4 md:my-6 p-4 md:p-6 rounded-xl border bg-white/5 shadow-inner backdrop-blur-sm"
    ...
    td: ({ node, ...props }: any) => (
      <td className="p-4 border-b border-white/5 text-gray-300 text-sm md:text-base hover:bg-white/5 transition-colors" {...props} />
    ),
        ...
        <div className="relative z-10 w-full pb-12 pt-32">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
            ...
            >
              ...
              {/* Description */}
              <p className="text-lg sm:text-xl md:text-2xl text-gray-300 leading-relaxed font-light max-w-3xl">
                {post.description}
              </p>
      ...
      <article className="relative z-10 pb-20 bg-black">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          <div className="grid lg:grid-cols-12 gap-12">
              ...
              >
                ...
                {(post.content_part1 || post.content_part2 || post.content_part3) ? (
                  ...
                  <ReactMarkdown
                    ...
                    components={MarkdownComponents}
                  >
                    {sanitizeHeadings(injectLinks(post.content || ''))}
                  </ReactMarkdown>
                )}
              </motion.div>
              ...
              <div className="mt-16 pt-8 border-t border-white/10">
                ...
                <div className="flex flex-wrap gap-3">
                  <Button
                    variant="outline"
                    size="sm"
                    className="border-white/10 hover:bg-[#1DA1F2]/10 hover:text-[#1DA1F2] hover:border-[#1DA1F2]/50 min-h-[44px]"
            ...
            {/* Sidebar — sticky directly on grid child, no wrapper */}
