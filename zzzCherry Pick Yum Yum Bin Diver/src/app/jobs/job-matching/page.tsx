      ...
      try {
        ...
        if (user?.id) {
          try {
            ...
            if (resumeCheck.ok) {
              const text = await resumeCheck.text();
              let resumeData;
              try {
                resumeData = JSON.parse(text);
              } catch (e) {
                console.error('Failed to parse resume check response:', text);
                resumeData = { hasSavedResume: false };
              }
        ...
        try {
          ...
          console.error('Failed to parse jobs API response:', text.substring(0, 100)); // Log first 100 chars
          throw new Error(`Invalid JSON response from /api/jobs/combined endpoint: ${res.status}`);
        }

        if (!data || !data.jobs) {
          console.warn('No jobs returned from API:', data);
          data = { jobs: [] };
        }
                    ...
                    <div className="absolute top-full left-0 mt-1 w-full bg-gray-800 border border-white/20 rounded-lg shadow-lg z-50">
                      {['all', 'remote', 'hybrid', 'onsite'].map((workType) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-sm hover:bg-white/10 transition-colors first:rounded-t-lg last:rounded-b-lg ${filterWorkType === workType ? 'bg-cyan-500/20 text-cyan-400' : 'text-white'
                            }`}
                        >
                    ...
                    <div className="absolute top-full left-0 mt-1 w-full bg-gray-800 border border-white/20 rounded-lg shadow-lg z-50">
                      {['all', 'day', 'night'].map((shift) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-sm hover:bg-white/10 transition-colors first:rounded-t-lg last:rounded-b-lg ${filterShift === shift ? 'bg-purple-500/20 text-purple-400' : 'text-white'
                            }`}
                        >
                          {getShiftLabel(shift)}
                        </button>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {job.priority && (
                              <Badge
                                className={`px-2 py-0.5 text-xs font-semibold ml-2 ${job.priority === 'urgent' ? 'bg-red-600/20 text-red-400 border-red-600/30' :
                                            ...
                                            return (
                                              <div key={key} className={`${getScoreBg(score)} border rounded-md p-3 transition-all hover:scale-105`}>
                                                ...
                                                <div className="w-full bg-gray-700/50 rounded-full h-1.5">
                                                  <div
                                                    className={`h-1.5 rounded-full transition-all duration-500 ${score >= 90 ? 'bg-gradient-to-r from-emerald-400 to-emerald-500' :
