...
export async function POST(request: NextRequest) {
    try {
        ...
        const { onboardingId, medicalCertUrl, nbiClearanceUrl, medicalNotes } = await request.json();
        if (!medicalCertUrl) return NextResponse.json({ error: 'Medical certificate required' }, { status: 400 });
        if (!nbiClearanceUrl) return NextResponse.json({ error: 'NBI clearance required' }, { status: 400 });
        ...
        if (onboarding?.job_application_id) {
            try {
                await logApplicationActivity(onboarding.job_application_id, {
                    ...
                    description: 'Completed onboarding section: Medical Certificate',
                    metadata: { section: 'medical', onboarding_id: onboardingId, timestamp: new Date().toISOString() }
                });
            } catch (logError) {
        ...
        return NextResponse.json({ success: true, message: 'Medical certificate submitted', status: 'SUBMITTED' });
    } catch (error: any) {
