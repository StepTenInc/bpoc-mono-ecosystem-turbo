...
export async function POST(request: NextRequest) {
  try {
    ...
    const { token, applicationId, proposedTimes, message, testMode } = body;

    // Check for development/test mode
    const isTestMode = testMode === true || process.env.NODE_ENV === 'development';

    // Validate required fields
    if (!token || !applicationId || !proposedTimes || !Array.isArray(proposedTimes) || proposedTimes.length < 1) {
      return NextResponse.json(
        { error: 'Token, applicationId, and at least 1 proposed time are required' },
        { status: 400 }
      );
    }
