...
export async function GET(
  ...
  try {
    ...
    const { data: counterOffers } = offerIds.length > 0
      ? await supabaseAdmin
          .from('counter_offers')
          .select('*')
    ...
    const roomIds = (preScreenRooms || []).map(r => r.id);
    
    const { data: recordings } = roomIds.length > 0 
      ? await supabaseAdmin
    ...
    const { data: transcripts } = roomIds.length > 0
      ? await supabaseAdmin
    ...
    const response = {
      ...
      onboarding: onboarding ? {
        id: onboarding.id,
        candidateName: `${onboarding.first_name} ${onboarding.middle_name || ''} ${onboarding.last_name}`.trim(),
