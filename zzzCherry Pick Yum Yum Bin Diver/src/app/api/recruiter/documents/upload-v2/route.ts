...
function crossCheckDocuments(docs: ExtractedDocData[]): {
  ...
  const names = docs.map(d => d.companyName?.toLowerCase().replace(/[^a-z0-9]/g, '')).filter(Boolean);
  const uniqueNames = [...new Set(names)];
  const companyNameConsistent = uniqueNames.length <= 1;
  if (!companyNameConsistent) {
    issues.push(`Company name mismatch: ${docs.map(d => d.companyName).filter(Boolean).join(' vs ')}`);
  }

  // Best company name (prefer SEC/DTI)
  const companyName = docs.find(d => d.documentType?.includes('SEC') || d.documentType?.includes('DTI'))?.companyName
    || docs.find(d => d.companyName)?.companyName || null;
