...
export async function GET(request: NextRequest) {
  try {
    ...
    const jobsWithCounts = await Promise.all(
      (jobs || []).map(async (job) => {
        ...
        return {
          ...
          aiValidatedAt: job.ai_validated_at,
    ...
    const stats = {
      ...
      aiApproved: jobsWithCounts.filter(j => j.approvalStatus === 'approved').length,
      pendingReview: jobsWithCounts.filter(j => j.approvalStatus === 'pending_review').length,
      rejected: jobsWithCounts.filter(j => j.approvalStatus === 'rejected' || j.status === 'rejected').length,
      withApplicants: jobsWithCounts.filter(j => j.applicantsCount > 0).length,
      totalApplicants: jobsWithCounts.reduce((sum, j) => sum + j.applicantsCount, 0),
    };
