...
export async function GET() {
  try {
    ...
    const [
      ...
      supabaseAdmin.from('job_applications').select('id', { count: 'exact', head: true }).in('status', ['submitted', 'under_review']),
      
      // Scheduled interviews
      supabaseAdmin.from('job_interviews').select('id', { count: 'exact', head: true }).eq('status', 'scheduled'),
      
      // Pending offers
      supabaseAdmin.from('job_offers').select('id', { count: 'exact', head: true }).eq('status', 'sent'),
