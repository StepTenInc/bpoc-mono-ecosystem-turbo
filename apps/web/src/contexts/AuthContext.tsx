    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (session?.user && event === 'SIGNED_IN' && typeof window !== 'undefined') {
          ...
          const isOnResumePage = currentPath.startsWith('/resume/') // Public resume pages should not redirect
          const isOnTestPage = currentPath.startsWith('/test') // Test pages should not redirect
          const isOnPublicPage = currentPath.startsWith('/resume/') || currentPath === '/' || currentPath.startsWith('/home')


          // Redirect ADMIN users to admin dashboard
          if (adminLevel === 'admin') {
            if (!isOnAdminPage && !isOnResumePage && !isOnTestPage) {
          ...
          // Redirect RECRUITER users to recruiter dashboard
            ...
            if (!isOnRecruiterPage && !isOnResumePage && !isOnTestPage) {
