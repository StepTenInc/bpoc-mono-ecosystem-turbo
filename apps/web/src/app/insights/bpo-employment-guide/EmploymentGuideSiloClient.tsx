...
import {
  ...
  ArrowLeft, ChevronRight, FileText, BookOpen,
  Crown, User, ScrollText, Scale, Shield, Stamp,
  BookMarked, ListChecks, Gavel, FileCheck
} from 'lucide-react';
...
// Slate color for employment guide silo
const siloColor = '#64748B';

// Markdown components for pillar content - employment guide theme
  ...
  h2: ({ node, ...props }: any) => (
    <div className="mt-16 mb-8 group">
      <h2 className="text-3xl font-bold text-white border-b border-white/10 pb-4 flex items-center gap-3 relative" {...props}>
        <span className="absolute -left-6 top-1 w-1 h-8 rounded-full opacity-100 shadow-lg bg-slate-400" />
        <span className="bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400">{props.children}</span>
      </h2>
    </div>
  ),
  h3: ({ node, ...props }: any) => (
    <h3 className="text-2xl font-bold text-slate-400 mt-10 mb-4 tracking-wide flex items-center gap-2" {...props} />
  ),
  p: ({ node, ...props }: any) => (
    <p className="text-gray-300 leading-relaxed mb-6 text-lg font-light tracking-wide" {...props} />
  ),
  ul: ({ node, ...props }: any) => (
    <ul className="space-y-4 my-6 p-6 rounded-xl border border-slate-500/30 bg-white/5 shadow-inner backdrop-blur-sm" {...props} />
  ),
  li: ({ node, ...props }: any) => (
    <li className="flex items-start gap-3 text-gray-300 group transition-colors duration-200 hover:text-white">
      <span className="mt-2 w-1.5 h-1.5 rounded-full bg-slate-400 shrink-0 shadow-lg" />
      <span className="flex-1 leading-relaxed">{props.children}</span>
    </li>
  ),
  blockquote: ({ node, ...props }: any) => (
    <div className="flex items-start gap-4 border-l-4 border-slate-400 p-5 rounded-r-xl my-6 bg-slate-500/10">
      <p className="text-gray-200 text-base leading-relaxed m-0">{props.children}</p>
    </div>
  ),
  a: ({ node, ...props }: any) => (
    <a className="font-medium transition-colors text-slate-400 underline underline-offset-4 decoration-2 decoration-slate-500/50 hover:decoration-slate-400" {...props} />
  ),
...
function ArticleCard({ article, siloSlug }: { article: any; siloSlug: string }) {
  ...
  return (
    <Link href={`/insights/${siloSlug}/${article.slug}`} className="h-full block">
      <div className="h-full flex flex-col relative group overflow-hidden rounded-xl border border-slate-500/20 bg-gradient-to-br from-slate-800/20 to-zinc-900/10 backdrop-blur-md transition-all duration-300 hover:-translate-y-2 hover:border-slate-400/40 hover:shadow-2xl hover:shadow-slate-500/20">
        {/* Document corner fold effect */}
        <div className="absolute top-0 right-0 w-12 h-12 z-20">
          <div className="absolute top-0 right-0 w-0 h-0 border-t-[48px] border-t-slate-700/50 border-l-[48px] border-l-transparent group-hover:border-t-slate-600/60 transition-colors" />
        </div>

        <div className="h-48 w-full relative overflow-hidden bg-gradient-to-br from-slate-800/30 to-zinc-900/20">
          {hasVideo ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <BookOpen className="w-20 h-20 text-slate-500/30" />
            </div>
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-[#0B0B0D] via-transparent to-transparent" />

          <div className="absolute top-4 left-4 z-10">
            <Badge className="bg-slate-500/20 backdrop-blur-md border-slate-400/30 text-slate-300">
        ...
        <div className="p-6 flex flex-col flex-grow relative z-10">
          ...
          <h3 className="text-xl font-bold text-white mb-3 group-hover:text-slate-300 transition-colors line-clamp-2">
            {article.title}
          </h3>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-500/10 mt-auto">
            ...
            <span className="text-sm font-medium text-slate-400 group-hover:text-slate-300 flex items-center">
...
export default function EmploymentGuideSiloClient({ silo, pillarPost, articles, pagination, relatedSilos }: EmploymentGuideSiloClientProps) {
  ...
  return (
    <div className="overflow-x-hidden selection:bg-slate-500/20 selection:text-slate-200 font-sans min-h-screen bg-[#0B0B0D]">
      <Header />

      {/* EMPLOYMENT GUIDE THEME: Official/Document aesthetic with formal tones */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute top-[-20%] left-1/3 w-[800px] h-[800px] bg-slate-500/5 rounded-full blur-[150px]" />
        <div className="absolute top-[40%] right-[-5%] w-[600px] h-[600px] bg-zinc-500/5 rounded-full blur-[120px]" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[700px] h-[700px] bg-gray-500/5 rounded-full blur-[140px]" />

        {/* Document lines pattern */}
        <div className="absolute inset-0 opacity-[0.02]">
          {[...Array(20)].map((_, i) => (
            <div key={i} className="absolute left-[10%] right-[10%] h-px bg-slate-400" style={{ top: `${5 + i * 5}%` }} />
          ))}
        </div>
      </div>
      ...
      <div className="pt-24 pb-20 relative z-10">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 text-sm text-gray-500 mb-8">
            ...
            <span className="text-slate-400 font-semibold">Employment Guide</span>
          </motion.div>

          {/* HERO - Official Document/Legal themed */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
            <div className="relative overflow-hidden rounded-3xl border-2 border-slate-500/30 bg-gradient-to-br from-slate-800/30 via-zinc-900/20 to-gray-900/10 backdrop-blur-xl">
              {/* Official seal decoration */}
              <div className="absolute top-8 right-8 opacity-10">
                <div className="w-24 h-24 rounded-full border-4 border-slate-400 flex items-center justify-center">
                  <Stamp className="w-12 h-12 text-slate-400" />
                </div>
              </div>
              ...
              <div className="relative p-8 md:p-16">
                <div className="flex flex-col lg:flex-row items-start gap-8">
                  <motion.div
                    ...
                    className="w-28 h-28 rounded-3xl flex items-center justify-center bg-gradient-to-br from-slate-400 to-zinc-600 shadow-2xl shadow-slate-500/30"
                  >
                    <BookOpen className="w-16 h-16 text-white" />
                  </motion.div>

                  <div className="flex-1">
                    <Badge className="mb-4 bg-slate-500/20 text-slate-300 border-slate-400/40 text-sm px-4 py-1">
                    ...
                    <h1 className="text-5xl md:text-6xl lg:text-7xl font-black mb-6 tracking-tight">
                      ...
                      <br />
                      <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-300 via-zinc-400 to-gray-500">
                        Rights.
                      </span>
                    </h1>

                    <p className="text-xl text-gray-300 max-w-2xl leading-relaxed mb-8">
                      Complete employment guides covering contracts, benefits, labor laws, and workplace policies.
                      <span className="text-slate-400 font-semibold"> Be informed. Be protected.</span>
                    </p>

                    <div className="flex flex-wrap items-center gap-6 text-sm">
                      <div className="flex items-center gap-2 bg-slate-500/10 px-4 py-2 rounded-full border border-slate-500/20">
          ...
          {pillarPost && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-16">
              <div className="rounded-2xl border border-slate-500/20 bg-gradient-to-br from-slate-800/10 to-transparent backdrop-blur-xl overflow-hidden">
                <div className="p-6 border-b border-slate-500/20 bg-gradient-to-r from-slate-800/30 to-transparent">
                  <div className="flex items-center gap-3 mb-3"><Crown className="w-6 h-6 text-yellow-400" /><Badge className="bg-slate-500/20 text-slate-300 border-slate-400/30">Complete Employment Manual</Badge></div>
                ...
                <div className="p-6 md:p-8">
                    ...
                    {(pillarPost.content_part1 || pillarPost.content_part2 || pillarPost.content_part3) ? (
                      <>
                        {pillarPost.content_image0 && (
                          <figure className="my-8 rounded-2xl overflow-hidden border border-slate-500/20 shadow-2xl">
                        ...
                        {pillarPost.content_image1 && (
                          <figure className="my-12 rounded-2xl overflow-hidden border border-slate-500/20 shadow-2xl">
                  ...
                  <div className="mt-4 md:hidden"><Button onClick={() => setShowFullPillar(!showFullPillar)} variant="outline" className="w-full border-slate-500/30 text-slate-400 hover:bg-slate-500/10">{showFullPillar ? 'Show Less' : 'Read Full Guide'}</Button></div>
                </div>
              </div>
            </motion.div>
          )}
          ...
          {displayedArticles.length > 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mb-8">
              <h2 className="text-2xl font-bold text-white flex items-center gap-3"><ListChecks className="w-6 h-6 text-slate-400" />Employment Resources</h2>
              <p className="text-gray-500 mt-1">{pagination.total} comprehensive guides for BPO employees</p>
            </motion.div>
          )}

          {displayedArticles.length === 0 && !pillarPost ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
              <BookOpen className="w-20 h-20 mx-auto text-slate-500/20 mb-4" />
              <h2 className="text-xl font-semibold text-white mb-2">No employment guides yet</h2>
              <p className="text-gray-500">Check back soon for employment resources.</p>
            </motion.div>
          ) : (
            <>
              ...
              {pagination.hasMore && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mt-12"><Button onClick={loadMore} disabled={loading} className="px-8 py-3 bg-slate-500/10 border border-slate-500/30 hover:bg-slate-500/20 text-slate-400">{loading ? 'Loading...' : 'Load More Guides'}</Button></motion.div>)}
            </>
          )}
