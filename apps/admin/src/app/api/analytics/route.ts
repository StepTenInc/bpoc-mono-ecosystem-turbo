...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: thisMonthOffers } = await supabase
      .from('job_offers')
    ...
    const agencyPerformance = await Promise.all(
      (agencies || []).map(async (agency) => {
        ...
        const { data: agencyOffers } = await supabase
          .from('job_offers')
          .select('id, salary_offered')
    ...
    const recentPlacements = (recentOffers || []).map(offer => {
      ...
      return {
        id: offer.id,
        candidate_name: app ? candidateMap[app.candidate_id] || 'Unknown' : 'Unknown',
        ...
        hired_at: offer.accepted_at || offer.created_at,
      };
    });
    ...
    return NextResponse.json({
      ...
      agency_performance: agencyPerformance.slice(0, 10),
