...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: applicationsWithRecruiters } = await supabase
      .from('job_applications')
      .select(`
        id,
        reviewed_by,
    ...
    filteredJobs.forEach(job => {
      const client = Array.isArray(job.agency_client) ? job.agency_client[0] : job.agency_client;
      const agency = client?.agency ? (Array.isArray(client.agency) ? client.agency[0] : client.agency) : null;
    ...
    applications?.forEach(app => {
      ...
      const client = job?.agency_client ? (Array.isArray(job.agency_client) ? job.agency_client[0] : job.agency_client) : null;
      const agency = client?.agency ? (Array.isArray(client.agency) ? client.agency[0] : client.agency) : null;
      const agencyId = agency?.id;
      if (agencyId) {
    ...
    const topAgenciesByActivity = Array.from(agencyActivity.values())
      ...
      .map(agency => ({
        ...
        total_activity: agency.applications + agency.placements,
      }));

    return NextResponse.json({
      top_jobs_by_applications: topJobsByApplications,
      top_jobs_by_placements: topJobsByPlacements,
      top_recruiters_by_placements: topRecruitersByPlacements,
      top_agencies_by_activity: topAgenciesByActivity,
    });
