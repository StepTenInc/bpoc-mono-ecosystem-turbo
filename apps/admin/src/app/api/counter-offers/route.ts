  ...
  try {
    ...
    const formattedCounterOffers = filteredCounterOffers.map(counterOffer => {
      ...
      return {
        ...
        salary_type: offer?.salary_type || 'month',
        candidate_message: counterOffer.candidate_message,
        employer_response: counterOffer.employer_response,
        response_type: counterOffer.response_type,
        created_at: counterOffer.created_at,
        responded_at: counterOffer.responded_at
      };
    });

    // Calculate stats
    const { data: allCounterOffers } = await supabase
      .from('counter_offers')
    ...
    return NextResponse.json({
      counter_offers: formattedCounterOffers,
      stats: {
        ...stats,
        average_increase: parseFloat(stats.averageIncrease.toFixed(1)),
        acceptance_rate: parseFloat(stats.acceptanceRate.toFixed(1))
      },
      pagination: {
        page,
        limit,
