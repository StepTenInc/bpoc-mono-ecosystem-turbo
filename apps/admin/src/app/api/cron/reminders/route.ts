...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: interviews15m, error: error15m } = await supabase
      .from('job_interviews')
      .select(`
        id,
        scheduled_at,
    ...
    // ========================================
    const { data: expiringOffers, error: errorOffers } = await supabase
      .from('job_offers')
      .select(`
        id,
        expires_at,
    ...
    const { data: startingTomorrow, error: errorDayOne } = await supabase
    ...
    const { data: missedCalls, error: errorMissed } = await supabase
