    ...
    const candidatesWithData = await Promise.all(
      (candidates || []).map(async (candidate) => {
        ...
        return {
          ...
          phone: candidate.candidate_profiles?.[0]?.phone || null,
          avatar: candidate.avatar_url,
          location: candidate.candidate_profiles?.[0]?.location || null,
          has_resume: (resumeResult.data?.length || 0) > 0,
          has_ai_analysis: (analysisResult.data?.length || 0) > 0,
          ai_score: analysisResult.data?.[0]?.overall_score || null,
