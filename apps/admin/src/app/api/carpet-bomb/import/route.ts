    ...
    for (let i = 0; i < parsed.rows.length; i++) {
      ...
      try {
        ...
        const { data: existingLead } = await supabase
          ...
          .select('id, email, first_name, last_name, phone_number, city, current_salary, expected_salary, resume_url, profile_picture_url')
          .eq('email', email)
          .single() as { data: { id: string; email: string; first_name: string | null; last_name: string | null; phone_number: string | null; city: string | null; current_salary: number | null; expected_salary: number | null; resume_url: string | null; profile_picture_url: string | null } | null };
