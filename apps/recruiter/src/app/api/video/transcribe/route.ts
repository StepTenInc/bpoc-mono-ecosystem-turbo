 ...
 */
async function convertToMp3WithCloudConvert(video_url: string): Promise<Buffer> {
  ...
  console.log('✅ [CloudConvert] Job created:', job_id);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { recording_id, room_id, audioUrl, source, transcript_id } = body;
    ...
    if (recording_id) {
      ...
      if (error || !data) {
        console.error('❌ [Transcribe] Recording not found:', recording_id);
      ...
      if (error || !data) {
        console.error('❌ [Transcribe] No recording found for room:', room_id);
    ...
    // Check if transcript already exists (use transcript_id from webhook if provided)
    ...
    const { data: transcriptRecord, error: createError } = await supabaseAdmin
      .from('video_call_transcripts')
      .upsert({
        id: existingTranscript?.id || transcript_id,
        room_id: room.id,
    ...
    const transcript_idToUpdate = transcriptRecord?.id || existingTranscript?.id || transcript_id;
    ...
    if (updatedTranscript && room.agency_id && room.application_id) {
      webhookVideoTranscriptCompleted({
        transcript_id: updatedTranscript.id,
        recording_id: recording.id,
        room_id: room.id,
