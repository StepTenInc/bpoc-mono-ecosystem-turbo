...
export async function POST(request: NextRequest) {
  try {
    ...
    const { room_id, roomName } = body;

    if (!room_id && !roomName) {
      return NextResponse.json({ error: 'room_id or roomName is required' }, { status: 400 });
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      message: 'POST to this endpoint with { room_id } or { roomName } to sync recordings',
