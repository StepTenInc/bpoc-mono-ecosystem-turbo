...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('📬 [Daily Webhook] Parsed event:', {
      ...
      recording_id: event.recording?.id || event.payload?.recording_id,
    });
...
async function handleRecordingReady(event: any) {
  ...
  console.log('✅ [Daily Webhook] Found room in DB:', { room_id: room.id, enableTranscription: room.enable_transcription });
...
async function queueTranscriptionJob(
  ...
  console.log('📝 [Daily Webhook] Queueing transcription for room:', room_id);

  // Best-effort job id inference if missing
  let inferredJobId: string | null | undefined = jobId;
  if (!inferredJobId && application_id) {
  ...
  console.log('✅ [Daily Webhook] Transcript job queued:', { transcript_id: transcript.id, room_id, recording_id, hasAudioUrl: !!audioUrl });
}
