...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      if (res.error) {
        console.error('[video/rooms/:room_id/join] Participant upsert failed (non-fatal):', res.error.message);
      }
    } catch (e) {
      console.error('[video/rooms/:room_id/join] Failed to upsert participant (non-fatal):', e);
    }
