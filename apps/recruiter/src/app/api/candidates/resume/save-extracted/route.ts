 ...
 */
function generateResumeSlug(first_name: string, last_name: string, userId: string): string {
  // Clean and normalize names
  const cleanFirst = (first_name || 'user')
    .toLowerCase()
  ...
  const cleanLast = (last_name || 'profile')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]/g, '')
 ...
 */
function extractNameFromResumeData(resumeData: any): { first_name: string; last_name: string; fullName: string } {
  ...
  // Final fallback
  return { first_name: 'user', last_name: 'profile', fullName: 'user profile' };
}
 ...
 */
async function uploadResumeFile(fileData: string, file_name: string, candidate_id: string): Promise<string | null> {
  try {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { resumeData, originalFileName, candidate_id, fileUrl, fileData } = body;
    ...
    if (!resumeData.bestJobTitle && !resumeData.position) {
      ...
      if (resumeData.experience && Array.isArray(resumeData.experience) && resumeData.experience.length > 0) {
        ...
        resumeData.bestJobTitle = mostRecentExp.title || mostRecentExp.position || mostRecentExp.job_title || null;
