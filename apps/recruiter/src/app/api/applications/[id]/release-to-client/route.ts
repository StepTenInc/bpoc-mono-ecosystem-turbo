...
export async function POST(
  ...
  try {
    ...
    if ((application.job as any)?.agency_client?.[0]?.agency_id !== recruiter.agency_id) {
      return NextResponse.json(
    ...
    const { data: jobToken } = await supabaseAdmin
      ...
      .eq('agency_client_id', (application.job as any)?.agency_client_id)
      .single();
    ...
    if (generateDirectLink) {
      try {
        const candidateTokenData = await generateCandidateToken(
          application_id,
          (application.job as any)?.agency_client_id,
          userId,
          jobToken.token,
    ...
    if (sendEmail && (application.job as any)?.agency_client?.[0]?.contact_email) {
      try {
        ...
        await sendClientCandidateReleasedEmail(
          (application.job as any)?.agency_client?.[0]?.contact_email,
          (application.job as any)?.agency_client?.[0]?.contact_name || 'Client',
          `${(application.candidate as any)?.first_name} ${(application.candidate as any)?.last_name}`,
          (application.candidate as any)?.headline || 'Candidate',
        ...
        console.log('Candidate released email sent to:', (application.job as any)?.agency_client?.[0]?.contact_email);
      } catch (emailError) {
    ...
    return NextResponse.json({
      ...
      application: {
        ...
        released_to_client: updatedApplication.released_to_client,
        released_at: updatedApplication.released_at,
        released_by: updatedApplication.released_by,
      },
      jobDashboardUrl,
      candidateDirectLink,
      emailSent: sendEmail && !!(application.job as any)?.agency_client?.[0]?.contact_email,
    });
  } catch (error) {
