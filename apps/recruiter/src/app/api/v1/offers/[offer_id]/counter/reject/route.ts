...
/**
 * POST /api/v1/offers/:offer_id/counter/reject
...
export async function POST(
  ...
  try {
    ...
    const { data: offer } = await supabaseAdmin
      ...
      .eq('id', offer_id)
      .eq('job_applications.jobs.agency_clients.agency_id', auth.agency_id)
      .maybeSingle();
    ...
    const application_id = (offer as any)?.application_id as string;
    ...
    if (!counterOffer || counterOffer.offer_id !== offer_id) {
    ...
    if (candidateId) {
      await supabaseAdmin.from('notifications').insert({
        ...
        action_url: `/candidate/offers/${offer_id}`,
