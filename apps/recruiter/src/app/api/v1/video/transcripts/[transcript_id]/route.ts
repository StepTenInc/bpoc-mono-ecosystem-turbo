...
/**
 * GET /api/v1/video/transcripts/[transcript_id]
...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ transcript_id: string }> }
) {
  ...
  const { transcript_id } = await params;
  ...
  try {
    ...
    return withCors(NextResponse.json({
      ...
      interview: {
        room_id: transcript.video_call_rooms?.id,
