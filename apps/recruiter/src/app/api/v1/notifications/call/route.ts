...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!room_id || !candidateId || !participantJoinUrl) {
            console.error('[Notification API] Missing required fields');
            return NextResponse.json(
                { error: 'Missing required fields: room_id, candidate_id, participantJoinUrl' },
        ...
        if (!isValidUUID(room_id)) {
            console.error('[Notification API] Invalid room_id format:', room_id);
            return NextResponse.json(
                { error: 'Invalid room_id format. Expected UUID' },
                { status: 400, headers: corsHeaders }
            );
        }
        ...
        if (!roomRecord) {
            ...
            if (!roomNameFromUrl || roomNameFromUrl.length < 3) {
                roomNameFromUrl = `shoreagents-${room_id.replace(/-/g, '').substring(0, 16)}`;
            }
