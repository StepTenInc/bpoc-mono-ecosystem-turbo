...
export async function POST(request: NextRequest) {
  try {
    ...
    const { application_id, salary, currency, startDate, benefits, message, token } = body;

    if (!application_id || !salary || !startDate || !token) {
      return NextResponse.json(
        { error: 'Missing required fields: application_id, salary, startDate, token' },
        { status: 400 }
      );
    }
