...
export async function POST(
  ...
  try {
    ...
    try {
      const { data: webhookJob } = await supabaseAdmin
        .from('jobs')
        .select('agency_client_id')
        .eq('id', application.job_id)
        .single();

      if (webhookJob?.agency_client_id) {
        const { data: webhookAgencyClient } = await supabaseAdmin
          .from('agency_clients')
          .select('agency_id')
          .eq('id', webhookJob.agency_client_id)
          .single();

        if (webhookAgencyClient?.agency_id) {
          webhookApplicationStatusChanged({
            ...
            newStatus: 'withdrawn',
            changedBy: user.id,
            agencyId: webhookAgencyClient.agency_id,
          }).catch(err => console.error('[Webhook] Decline invite error:', err));
        }
      }
    } catch (e) {
      console.warn('[Decline Invite API] Failed to trigger webhook:', e);
    }
