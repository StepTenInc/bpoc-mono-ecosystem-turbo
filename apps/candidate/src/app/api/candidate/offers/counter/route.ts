...
export async function POST(request: NextRequest) {
  try {
    ...
    // Support both snake_case (new) and camelCase (legacy) for backwards compatibility
    const offerId = body.offer_id || body.offerId;
    const requestedSalary = body.requested_salary || body.requestedSalary;
    const candidateMessage = body.candidate_message || body.candidateMessage;

    if (!offerId || !requestedSalary) {
      return NextResponse.json(
        { error: 'offer_id and requested_salary are required' },
        { status: 400 }
      );
    }
