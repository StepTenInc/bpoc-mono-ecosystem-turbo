...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedOffers = (offers || []).map((offer) => {
      const app = offer.application as unknown as {
        id: string;
        job?: {
          id: string;
          title: string;
      ...
      const counterOffers = (offer.counter_offers || []).map((co: any) => ({
        id: co.id,
        requested_salary: co.requested_salary,
        requested_currency: co.requested_currency,
        candidate_message: co.candidate_message,
        employer_response: co.employer_response,
      ...
      const latest_counter = counterOffers.length > 0
