  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      switch (task.task_type) {
        ...
        case 'acknowledgment':
        case 'information':
          if (!acknowledged && task.task_type === 'acknowledgment') throw new Error('Please acknowledge to continue');
      ...
      <DialogContent className="bg-[#0F1419] border-white/10 text-gray-300 sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-white flex items-center gap-2">
            {task.task_type === 'document_upload' && <Upload className="h-5 w-5 text-orange-500" />}
            {task.task_type === 'form_fill' && <FileText className="h-5 w-5 text-cyan-500" />}
            {task.task_type === 'acknowledgment' && <CheckCircle className="h-5 w-5 text-emerald-500" />}
            {task.title}
          </DialogTitle>
