...
export default function CounterOfferModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/candidate/offers/counter', {
        ...
        body: JSON.stringify({
          offer_id: offer.id,
          requested_salary: parseFloat(requestedSalary),
  ...
  const differencePercent = requestedSalary
    ? (((parseFloat(requestedSalary) - offer.salary_offered) / offer.salary_offered) * 100).toFixed(1)
    : '0';
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">{offer.job_title} at {offer.company}</p>
            </div>
          </div>
          <Button
            variant="ghost"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="p-4 rounded-lg bg-white/5 border border-white/10">
            ...
            <p className="text-2xl font-bold text-white">
              {offer.currency} {offer.salary_offered.toLocaleString()}
          ...
          <div>
            ...
            <div className="relative mt-2">
              ...
              <Input
                id="requestedSalary"
                type="number"
                placeholder={`e.g. ${Math.round(offer.salary_offered * 1.1)}`}
                ...
                min={offer.salary_offered}
              />
            </div>
            {requestedSalary && parseFloat(requestedSalary) > offer.salary_offered && (
              <p className="text-cyan-400 text-sm mt-2">
                <TrendingUp className="h-4 w-4 inline mr-1" />
                +{differencePercent}% increase ({offer.currency} {(parseFloat(requestedSalary) - offer.salary_offered).toLocaleString()} more)
              </p>
            )}
            {requestedSalary && parseFloat(requestedSalary) <= offer.salary_offered && (
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
            ...
            <Button
              type="submit"
              disabled={submitting || !requestedSalary || parseFloat(requestedSalary) <= offer.salary_offered}
